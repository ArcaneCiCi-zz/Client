...
public abstract class BlockFluids extends Block {
    ...
    private Vec3D func_291_e(IBlockAccess iblockaccess, int i, int j, int k) {
        ...
        if (iblockaccess.getBlockMetadata(i, j, k) >= 8) {
            boolean flag = shouldSideBeRendered(iblockaccess, i, j, k - 1, 2);
    ...
    public float getBlockBrightness(IBlockAccess iblockaccess, int i, int j, int k) {
        ...
        float f1 = Client.getInstance().getModManager().getFullbrightModule().isEnabled() ? 0.85F : iblockaccess.getLightBrightness(i, j + 1, k);
        return f <= f1 ? f1 : f;
    }
    ...
    private void checkForHarden(World world, int i, int j, int k) {
        ...
        if (blockMaterial == Material.lava) {
            boolean flag = world.getBlockMaterial(i, j, k - 1) == Material.water;
