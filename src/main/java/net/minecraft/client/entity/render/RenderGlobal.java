...
import net.minecraft.client.entity.mob.impl.*;
...
import net.minecraft.client.interfaces.IWorldAccess;
...
import net.minecraft.client.world.tile.TileEntityRenderer;
...
public class RenderGlobal implements IWorldAccess {
    ...
    private final List field_1446_m;
        ...
        for (int j1 = 0; j1 < field_1443_p; j1++) {
            for (int k1 = 0; k1 < field_1442_q; k1++) {
                for (int l1 = 0; l1 < field_1441_r; l1++) {
                    field_1444_o[(l1 * field_1442_q + k1) * field_1443_p + j1] = new WorldRenderer(worldObj, field_1458_a, j1 * 16, k1 * 16, l1 * 16, 16, field_1440_s + k);
                    if (field_1436_w) {
    ...
    public void func_951_a(Vec3D vec3d, ICamera icamera, float f) {
        ...
        field_1421_L = 0;
        EntityPlayerSP entityplayersp = mc.thePlayer;
        RenderManager.field_1232_b = entityplayersp.lastTickPosX + (entityplayersp.posX - entityplayersp.lastTickPosX) * (double) f;
        RenderManager.field_1231_c = entityplayersp.lastTickPosY + (entityplayersp.posY - entityplayersp.lastTickPosY) * (double) f;
        RenderManager.field_1230_d = entityplayersp.lastTickPosZ + (entityplayersp.posZ - entityplayersp.lastTickPosZ) * (double) f;
        ...
        for (int i1 = i; i1 < j; i1++) {
            if (k == 0) {
                ...
                if (field_1445_n[i1].field_1748_p[k]) {
                    field_1416_Q++;
                } else if (!field_1445_n[i1].field_1749_o) {
                    field_1419_N++;
                } else if (field_1436_w && !field_1445_n[i1].field_1734_x) {
                    field_1418_O++;
                } else {
                    field_1417_P++;
                }
    ...
    public boolean func_948_a(EntityPlayer entityplayer, boolean flag) {
        ...
        for (int k = 0; k < j; k++) {
            ...
            } else if (!worldrenderer.field_1749_o) {
                continue;
            }
    ...
    public void playSound(String s, double d, double d1, double d2,
        ...
        if (mc.thePlayer.getDistanceSq(d, d1, d2) < (double) (f2 * f2)) {
            mc.sndManager.func_336_b(s, (float) d, (float) d1, (float) d2, f, f1);
        }
    }
    ...
    public void obtainEntitySkin(Entity entity) {
        ...
        if (entity.capeUrl != null) {
            renderEngine.obtainImageData("http://textures.minecraft.net/texture/e7dfea16dc83c97df01a12fabbd1216359c0cd0ea42f9999b6e97c584963e980", new ImageBufferDownload());
        }
    }
