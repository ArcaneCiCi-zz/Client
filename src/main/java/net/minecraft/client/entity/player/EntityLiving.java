...
public class EntityLiving extends Entity {
    ...
    public void handleMovement(float f, float f1) {
        if (handleWaterMovement()) {
            double d = posY;
            moveFlying(f, f1, 0.02F);
            moveEntity(motionX, Client.getInstance().getModManager().getFlyModule().isEnabled() ? 0.0D : motionY, motionZ);
            ...
            if (!Client.getInstance().getModManager().getFlyModule().isEnabled()) motionY *= 0.5D;
            if (!Client.getInstance().getModManager().getFlyModule().isEnabled()) motionY -= 0.02D;

            motionZ *= 0.5D;
            if (!Client.getInstance().getModManager().getFlyModule().isEnabled()) {
    ...
    public void onLivingUpdate() {
        ...
        field_9340_ai *= 0.98F;
        field_9338_aj *= 0.9F;
        handleMovement(field_9342_ah, field_9340_ai);
    ...
    protected void func_418_b_() {
        ...
        if (rand.nextFloat() < 0.02F) {
            ...
            if (entityplayer1 != null) {
                ...
                field_9338_aj = (rand.nextFloat() - 0.5F) * 20F;
            }
        }
