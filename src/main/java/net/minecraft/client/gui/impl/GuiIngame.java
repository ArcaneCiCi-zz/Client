...
public class GuiIngame extends Gui {
    ...
    public void renderGameOverlay(float f, boolean flag, int i, int j) {
        ...
        if (Keyboard.isKeyDown(61)) {
            fontrenderer.drawStringWithShadow((new StringBuilder()).append("Minecraft Alpha v1.2.6 (").append(mc.debugInfo).append(")").toString(), 2, 2, 0xffffff);
            ...
            fontrenderer.drawStringWithShadow(Client.getInstance().getModManager().getXrayModule().displayTypes.get(Client.getInstance().getModManager().getXrayModule().currentXrayType), 2, 13, 0xffffff);
            int count = 0;
            for (Module mod : Client.getInstance().getModManager().getMods().stream().sorted((mod, mod1) -> mod.getName().length() < mod1.getName().length() ? -1 : 0).collect(Collectors.toList())) {
                if (mod.isEnabled()) {
                    fontrenderer.drawStringWithShadow(mod.getName(), scaledresolution.getScaledWidth() - fontrenderer.getStringWidth(mod.getName()) - 2, 2 + count * 12, mod.getColor());
                    ++count;
                }
            }
        }
